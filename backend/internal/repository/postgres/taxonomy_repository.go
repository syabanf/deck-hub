...
const taxonomyColumns = `t.kind, t.slug, t.title, t.description, t.sort_order, t.active, t.accent, t.secondary, t.created_at, t.updated_at`
...
func (r *TaxonomyRepository) List(ctx context.Context, f domain.TaxonomyFilter) ([]*domain.TaxonomyTerm, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&t.Kind, &t.Slug, &t.Title, &t.Description, &t.SortOrder, &t.Active,
...
func (r *TaxonomyRepository) Get(ctx context.Context, kind domain.TaxonomyKind, slug string) (*domain.TaxonomyTerm, error) {
	...
	var t domain.TaxonomyTerm
	err := r.pool.QueryRow(ctx, q, kind, slug).Scan(&t.Kind, &t.Slug, &t.Title, &t.Description,
...
func (r *TaxonomyRepository) Create(ctx context.Context, t *domain.TaxonomyTerm) error {
	const q = `
		INSERT INTO taxonomy_terms (kind, slug, title, description, sort_order, active, accent, secondary)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
		RETURNING created_at, updated_at`

	err := r.pool.QueryRow(ctx, q, t.Kind, t.Slug, t.Title, t.Description, t.SortOrder,
...
func (r *TaxonomyRepository) Update(ctx context.Context, t *domain.TaxonomyTerm) error {
	const q = `
		UPDATE taxonomy_terms
		   SET title = $3, description = $4, sort_order = $5, active = $6, accent = $7,
		       secondary = $8, updated_at = now()
		 WHERE kind = $1 AND slug = $2
		RETURNING created_at, updated_at`
